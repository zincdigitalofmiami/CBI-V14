# Mac Training Setup Plan - M4 TensorFlow Metal Pipeline
...
The VERTEX_AI_TRUMP_ERA_PLAN.md outlines a neural forecasting pipeline (Phase 4.5) that trains TensorFlow/Keras models locally on the Apple M4 Mac mini using GPU acceleration via tensorflow-metal, then deploys to Vertex AI.
...
- **Hardware**: Apple M4 Mac mini with integrated GPU
- **Python**: 3.12.6 (pyenv virtualenv `vertex-metal-312`)
- **TensorFlow**: 2.16.2 (`tensorflow-macos` latest available)
- **TensorFlow Metal**: 1.2.0
- **Status**: ✅ Installed and verified (Apple M4 GPU detected)
...
- **Training**: Local Apple M4 Mac mini with GPU acceleration
...
#### 4.3 GPU Utilization
- **Batch Size**: Optimize for Apple M4 GPU memory
- **Mixed Precision**: Use float16 for faster training
- **Multi-GPU**: Not applicable (single Apple M4 GPU)
...
### Environment Setup
- [x] Verify Python 3.12.6 installed (`pyenv versions`, `python --version`)
- [x] Verify TensorFlow + Metal installed (`tensorflow-macos==2.16.2`, `tensorflow-metal==1.2.0`)
- [x] Test GPU acceleration (Metal device visible via TensorFlow)
- [x] Install required Python packages (keras, numpy, pandas, scikit-learn, shap, google-cloud-*)
- [x] Create pyenv virtualenv `vertex-metal-312`
...
- **Resource**: Apple M4 GPU is powerful but single device - optimize batch sizes accordingly
