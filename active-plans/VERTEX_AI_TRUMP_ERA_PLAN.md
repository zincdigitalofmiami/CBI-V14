...
**WHAT THIS IS:** A modular TensorFlow/Keras pipeline that trains multi-horizon neural forecasts (1M/3M/6M/12M) with Monte Carlo uncertainty quantification, SHAP explainability, and "what-if" scenario analysis. This pipeline complements Vertex AI AutoML by providing deep neural architectures (LSTM/GRU/Feedforward), real-time scenario injection, and interactive explainability for Chris's strategy page. Models are trained on the Apple M4 Mac mini (GPU via tensorflow-metal) and deployed to Vertex AI as custom SavedModel artifacts.
...
**SavedModel Export (`vertex-ai/deployment/export_savedmodel.py`):**
...
**Vertex AI Upload (`vertex-ai/deployment/upload_to_vertex.py`):**
...
**Endpoint Deployment (`vertex-ai/deployment/create_endpoint.py`):**
...
- **Scripts Executed:** 
  - `src/ingestion/trump_truth_social_monitor.py` (Trump Truth Social posts)
  - `src/ingestion/scrape_creators_full_blast.py` (Facebook, Twitter, LinkedIn intelligence)
...
- **Script Executed:** `src/ingestion/fred_economic_deployment.py` or `src/ingestion/economic_intelligence.py`
...
- **Script Executed:** `src/ingestion/ingest_usda_export_sales_weekly.py`
...
- **Script Executed:** `src/ingestion/ingest_epa_rin_prices.py`
...
- **Script Executed:** `src/ingestion/ingest_cftc_positioning_REAL.py`
...
- **Script Executed:** `src/ingestion/unified_weather_scraper.py`
