# CBI-V14 Complete PyTorch Implementation Guide

## Comprehensive Strategy for Commodity Price Forecasting

### Project Overview

CBI-V14 is a commodity price forecasting system that:
...
- Integrates multiple data sources (FRED, Yahoo, Alpha Vantage, NOAA, CFTC)
...
```python
"""
CBI-V14 PyTorch Implementation
...
class CBI_V14_Config:
    ...
    EXTERNAL_DRIVE_PATH = "/Volumes/Satechi Hub/Projects/CBI-V14/TrainingData/"
...
class BigQueryCommodityDataset(Dataset):
    ...
    Follows actual CBI-V14 architecture
    """
    ...
    def __init__(
        ...
        self.client = bq_client or bigquery.Client(project='cbi-v14')
    ...
    def _load_from_bigquery(self):
        ...
        Table already has all features joined (BQ SQL + Python + Alpha Vantage)
        """
        query = f"""
        SELECT *
        FROM `cbi-v14.training.zl_training_prod_allhistory_{self.horizon}`
    ...
    def _extract_features(self):
        ...
        Features come from:
        - Alpha Vantage: Pre-calculated technicals (RSI, MACD, etc.)
...
class CBI_V14_Deployment:
    ...
    def upload_predictions_to_bigquery(self, predictions_df: pd.DataFrame, horizon: str):
        ...
        table_id = 'cbi-v14.predictions.zl_predictions'
...
class CBI_V14_System:
    """
    Complete CBI-V14 production system
    """
...
3. **Medium-term**: Add Alpha Vantage technicals to feature pipeline
4. **Long-term**: Enhance with dual-stream LSTM (sentiment + prices), alternative data sources

---

*This implementation guide represents the complete PyTorch strategy for CBI-V14*
