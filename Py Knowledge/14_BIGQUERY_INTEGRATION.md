...
## Actual CBI-V14 Architecture (November 2025)

### Hybrid Python + BigQuery Pattern (Already in Production)

**CRITICAL**: CBI-V14 already uses a **hybrid system** - this is NOT theoretical, it's the actual production pattern.

```
Python Collection Scripts (FRED, Yahoo, Alpha Vantage)
    ↓
...
```sql
-- Correlations (Alpha Vantage doesn't have ZL futures)
...
-- Moving averages (if not using Alpha Vantage)
...
SELECT 
  ...
  rate_regime,
  -- Alpha Vantage technicals (pre-calculated, just join)
  av_rsi,
  av_macd,
  av_bbands_upper,
  ...
  policy_impact
FROM 
  raw_intelligence.yahoo_normalized zl
LEFT JOIN features.zl_correlations corr ON zl.date = corr.date
LEFT JOIN raw_intelligence.alpha_vantage_technicals av ON zl.date = av.date
...
├── yahoo_normalized (55 symbols)
├── alpha_vantage_technicals (NEW - pre-calculated)
├── alpha_vantage_options (NEW)
└── alpha_vantage_fx (NEW)
...
class AlphaVantageCollector:
    ...
    def collect(self):
        ...
        external_path = Path('/Volumes/Satechi Hub/Projects/CBI-V14/TrainingData/raw/alpha_vantage/')
...
**Alpha Vantage Provides** (50+ indicators):
```
...
class BigQueryDataset(Dataset):
    ...
    def __init__(self, horizon='1m', mode='train'):
        ...
        query = f"""
        SELECT *
        FROM `cbi-v14.training.zl_training_prod_allhistory_{horizon}`
    ...
    def _extract_features(self):
        ...
        feature_categories = {
            ...
            'technical': ['rsi', 'macd_signal', 'atr'],  # From Alpha Vantage
            'correlations': ['corr_zl_fed_30d', 'corr_zl_vix_30d'],  # From BQ SQL
            'regimes': ['rate_regime', 'volatility_regime'],  # From BQ SQL
            'substitution': ['palm_spread', 'palm_soybean_oil_spread'],  # From Yahoo
...
class CBI_V14_TrainingPipeline:
    ...
    def export_training_data(self, horizon='1m'):
        ...
        export_path = Path('/Volumes/Satechi Hub/Projects/CBI-V14/TrainingData/exports/')
    ...
    def upload_predictions(self, predictions, horizon='1m'):
        ...
        job = self.client.load_table_from_dataframe(
            predictions_df,
            'cbi-v14.predictions.zl_predictions',
            write_disposition='WRITE_APPEND'
        )
...
class DailyPipeline:
    ...
    def run_daily(self):
        ...
        # - Yahoo: collect_yahoo_finance_comprehensive.py → External drive + BQ
        # - Alpha Vantage: collect_alpha_vantage_master.py → External drive + BQ
        ...
        # - Alpha Vantage: Already in BQ (pre-calculated)
...
|--------------|---------------------|---------|
| **Alpha Vantage Technicals** | Alpha Vantage API (pre-calculated) | RSI, MACD, BBANDS, ATR |
...
```python
# 1. Load pre-calculated Alpha Vantage technicals
av_features = load_from_bq('raw_intelligence.alpha_vantage_technicals')
...
3. **Alpha Vantage**: Pre-calculated technicals (don't recalculate, use as-is)
