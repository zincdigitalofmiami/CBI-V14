...
SELECT 
  ...
  COUNT(*) as total_rows,
  COUNTIF(target_3m IS NOT NULL) as training_rows,
  ROUND(COUNTIF(target_3m IS NOT NULL) / COUNT(*) * 100, 1) as coverage_pct,
  MIN(target_3m) as min_target,
  MAX(target_3m) as max_target,
  ROUND(AVG(target_3m), 2) as mean_target,
  ROUND(STDDEV(target_3m), 2) as stddev_target,
  COUNTIF(target_3m < 0 OR target_3m > 100) as extreme_values,
  CASE 
    WHEN COUNTIF(target_3m IS NOT NULL) >= 1000 AND
         COUNTIF(target_3m < 0 OR target_3m > 100) = 0
...
SELECT 
  ...
  ROUND(COUNTIF(social_sentiment_avg IS NOT NULL) / NULLIF(COUNTIF(target_3m IS NOT NULL), 0) * 100, 1) as social_coverage,
  ROUND(COUNTIF(trump_policy_events IS NOT NULL) / NULLIF(COUNTIF(target_3m IS NOT NULL), 0) * 100, 1) as trump_coverage,
  ROUND(COUNTIF(cftc_commercial_extreme IS NOT NULL) / NULLIF(COUNTIF(target_3m IS NOT NULL), 0) * 100, 1) as cftc_coverage,
  ROUND(COUNTIF(usd_ars_rate IS NOT NULL) / NULLIF(COUNTIF(target_3m IS NOT NULL), 0) * 100, 1) as currency_coverage,
  CASE 
    WHEN COUNTIF(social_sentiment_avg IS NOT NULL) / NULLIF(COUNTIF(target_3m IS NOT NULL), 0) >= 0.9
...
SELECT 
  ...
  COUNTIF(target_3m IS NULL) as missing_target,
  COUNTIF(target_3m IS NOT NULL AND (target_3m < -100 OR target_3m > 100)) as extreme_target_values,
  ...
  CASE 
    WHEN COUNTIF(target_3m IS NOT NULL AND (target_3m < -100 OR target_3m > 100)) = 0 AND
...
SELECT 
  ...
  COUNTIF(target_3m IS NOT NULL) as training_rows,
  COUNT(*) as total_rows,
  ROUND(COUNTIF(target_3m IS NOT NULL) / COUNT(*) * 100, 1) as training_row_percentage,
  ...
  CASE 
    WHEN COUNTIF(target_3m IS NOT NULL) >= 1000
    THEN '✅ READY FOR TRAINING'
