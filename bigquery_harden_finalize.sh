...
echo "=== BIGQUERY HARDEN FINALIZE $(date -u) ==="

exists_table () { bq show --project_id="${PROJECT}" "${DATASET}.$1" >/dev/null 2>&1; }

# 1) soybean_prices_p  (partition: time; cluster: ticker, volume [valid types])
...
echo "-> Upserting soybean_prices -> soybean_prices_p (idempotent)"
...
echo "-> Upserting weather_data -> weather_data_p (idempotent)"
bq query --project_id="${PROJECT}" --use_legacy_sql=false --quiet "
...
# 3) fed_rates_p  (partition: time; cluster: time — valid)
echo -e "\n[3/4] fed_rates_p"
...
echo "-> Upserting fed_rates -> fed_rates_p (idempotent, quoting \`Cross\`)"
bq query --project_id="${PROJECT}" --use_legacy_sql=false --quiet "
...
echo "-> Upserting economic_indicators -> economic_indicators_p (idempotent)"
bq query --project_id="${PROJECT}" --use_legacy_sql=false --quiet "
