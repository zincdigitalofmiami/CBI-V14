...
export async function GET(request: NextRequest) {
  try {
    ...
    // ONLY calculate if we have real current price data
    const buyZones = []
    ...
    // Return error if no current price available - NO FAKE DATA
    if (current.length === 0 || !current[0]?.price) {
      return NextResponse.json({
        error: 'No current price data available',
        message: 'Cannot generate forward curve without current price',
        chartData: [],
        buyZones: [],
