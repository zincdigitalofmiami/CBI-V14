	...
	for _, fi := range fis {
		if strings.HasSuffix(fi.Name(), ".v1.count") {
			...
			if asof.IsZero() || reportDate.IsZero() {
				// Fall back on the old behavior of uploading all
				// unuploaded files if we can't be sure about dates.
				//
				// TODO(rfindley): After more testing confirms that report dates are
				// reliable, this logic should be inverted. We should only upload if
				// we can verify the report date against the `asof` date to strictly
				// respect the user's opt-in time.
				uploadable = true
			} else if asof.Before(reportDate) {
			...
			if uploadable {
				u.logger.Printf("Collecting for upload: %s", fi.Name())
