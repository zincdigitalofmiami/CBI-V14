...
See also: `HORIZON_TRAINING_STRATEGY.md` for the full horizon registry (active vs. research) and how special (intraday) and main (daily+) training combine.
See also: `docs/plans/BIGQUERY_CENTRIC_MIGRATION_PLAN.md` for the adopted BigQuery‑centric data plane, handoff contracts, and deprecations.
...
- Owner: Data Engineering
  - Enforce data authority: Prefer DataBento for overlapping futures; retain Yahoo ZL only pre‑2010; deprecate Alpha/Vendor daily bars where DataBento exists.

#### Procurement Signal Quality — Add Now (Day 1-2)
- Add DataBento roots to collection config (daily + intraday where noted):
  - Grains breadth: ZC, ZW, KE (daily OHLCV)
  - FX channels: 6L (BRL), CNH (Offshore RMB) (daily OHLCV)
  - Equity proxies (routers): NQ, MNQ, RTY, M2K (1m + daily OHLCV)
  - Energy spreads: CL, HO, RB (daily OHLCV)
 - Discontinued: CME CVOL indices (SOVL/SVL) — not used or collected.
- Extend MES orderflow ingestion (intraday):
  - Sources: MBP-10 depth, trades, quotes/TBBO via GLBX.MDP3
  - Target: enrich `market_data.orderflow_1m` with depth/imbalance; align trades/quotes with OHLCV.
...
Feature inputs to incorporate (ZL + MES):
 - ZL: grains breadth (ZC/ZW/KE), FX channels (6L/CNH), energy spreads (CL/HO/RB)
- MES: extended orderflow features from MBP-10/TBBO; equity proxies (NQ/RTY stacks) for router features
...
- Futures OHLCV (intraday + daily): DataBento authoritative for covered roots (2010→present). Yahoo ZL is a 2000–2010 bridge only. Deprecate Alpha/Vendor daily bars where DataBento exists.
- Macro/risk: FRED is canonical for rates, VIXCLS, credit. World Bank data may be used as a complementary macro source, ingested via `scripts/ingest/collect_worldbank_alternative.py`, but only for approved indicators and never as a silent replacement for FRED.
