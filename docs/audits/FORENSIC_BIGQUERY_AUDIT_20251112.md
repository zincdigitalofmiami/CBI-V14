...
**Reality Check:**
- ✅ `raw_intelligence` EXISTS (97 objects, heavily used)
- ✅ `training` EXISTS (92 objects, production training tables present)
...
**Row Count Reality:**
- `cftc_cot`: **944 rows** in `raw_intelligence` + **2,136 rows** in `training` (GOOD, not missing)
- `china_soybean_imports`: **22 rows** (LOW but EXISTS in `raw_intelligence`)
...
## 📊 DATASET INVENTORY (POST-REFACTORING VIEW)

### New Production Datasets
1. **raw_intelligence** (97 objects)
...
2. **training** (92 objects)  
   - **5 production training tables** (ALL EXIST):
     - `zl_training_prod_all_1w`: 1,472 rows
     - `zl_training_prod_all_1m`: 1,404 rows
     - `zl_training_prod_all_3m`: 1,475 rows
     - `zl_training_prod_all_6m`: 1,473 rows
     - `zl_training_prod_all_12m`: 1,473 rows
   - Historical regime tables (2000-2025): `zl_training_full_precrisis_all`, etc.
   - 16,824 rows in vertex_core_features (master time spine)
   - Feature importance, metadata, backtesting logs

3. **features** (Previously `signals` and parts of `models`)
    - Derived signal calculations and engineered features.

4. **predictions** 
    - Forecast outputs.

5. **monitoring**
    - Performance metrics and model registry.

6. **vegas_intelligence**
    - Sales intelligence dashboard data.

### Support & Legacy Datasets
7. **yahoo_finance_comprehensive** (9 objects)
...
- ✅ Related data EXISTS:
  - `raw_intelligence.argentina_crisis_tracker` (10 rows)
  - `training.argentina_port_logistics_daily` (1,347 rows) ← **THIS IS IT!**
  - `raw_intelligence.weather_brazil_clean` (33 rows)
  - `raw_intelligence.weather_brazil_daily` (0 rows - empty but exists)
...
- ✅ **4 ScrapeCreator views exist** in `raw_intelligence` (previously `forecasting_data_warehouse`):
...
- ✅ News data EXISTS in `raw_intelligence`:
...
1. **raw_intelligence.trade_china_soybean_imports: 22 rows** ⚠️
   - Expected: 500+ rows (monthly data 2017-2025)
   - Actual: 22 rows
   - **Status:** REAL GAP—needs backfill

2. **cftc_cot: 944 rows** ⚠️  
   - **BUT ALSO:** `training.cftc_daily_filled` has **2,136 rows** ✅
   - **AND:** `staging.cftc_cot` has 78 rows
   - Expected in `raw_intelligence`: 5,000+ rows (weekly 2006-2025)
   - **Status:** Data exists in `training` dataset; `raw_intelligence` table may be filtered subset

3. **Empty Tables (0 rows - 32 total):**
   - `predictions.current_forecasts` (0 rows)
   - `raw_intelligence.enso_climate_status` (0 rows)
   - `raw_intelligence.ers_oilcrops_monthly` (0 rows)
   - `raw_intelligence.futures_prices_*` (5 tables, all 0 rows)
   - `raw_intelligence.weather_argentina_daily` (0 rows)
   - `raw_intelligence.weather_brazil_daily` (0 rows)
   - `raw_intelligence.weather_us_midwest_daily` (0 rows)
   - Plus 20 more...
...
### Production Training Tables (in `training` dataset)
...
### Historical Regime Tables (in `training` dataset)
These **DO EXIST** and contain 2000-2025 data:

- `zl_training_full_precrisis_all`: 1,737 rows ✅
- `zl_training_full_crisis_all`: 253 rows ✅
- `zl_training_full_recovery_all`: 1,760 rows ✅
- `zl_training_full_tradewar_all`: 754 rows ✅
- `zl_training_prod_trump_all`: 732 rows ✅
...
2. `raw_intelligence.trade_china_soybean_imports` has only 22 rows (need backfill)
3. 32 empty tables (may be staging/deprecated; needs classification)
4. `news_data` dataset does not exist (but data exists in `raw_intelligence`)
5. Training tables lack pre-2020 data (but historical regime tables exist separately in `training`)

### ❌ FALSE ALARMS (Not Actually Missing)
1. `raw_intelligence` dataset (EXISTS, 97 objects)
2. `training` dataset (EXISTS, 92 objects)
3. `zl_training_prod_all_*` tables (ALL 5 EXIST and current)
4. `cftc_cot` data (EXISTS with 944+2,136 rows across datasets)
5. `argentina_exports` (EXISTS as `raw_intelligence.argentina_port_logistics_daily`)
6. News data (EXISTS in `raw_intelligence`, not in separate dataset)

### ⚠️ NEEDS INVESTIGATION
1. ScrapeCreator views (exist but need to verify if functional)
2. Schema consistency (date column naming—not yet verified)
3. Why CFTC data is split across 3 tables (`raw_intelligence`: 944, `training`: 2,136, staging: 78)
...
7. **Consider merging historical regime tables into production training tables** in the `training` dataset. 
...
2. **Data organization is complex but functional**
   ...
   - CFTC data is split for good reason (`raw_intelligence` = raw, `training` = processed daily)
...
WHERE table_schema IN ('raw_intelligence', 'training', 'features')
