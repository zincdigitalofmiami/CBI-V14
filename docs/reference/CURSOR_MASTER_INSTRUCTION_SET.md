...
**Purpose**: Cleaned, aggregated, validated features
...
```sql
-- Count nulls in all curated tables
SELECT 
  table_name,
  column_name,
  COUNT(*) as null_count
FROM `curated.INFORMATION_SCHEMA.COLUMNS` c
JOIN `curated.{table}` t ON TRUE
WHERE t.signal_date >= CURRENT_DATE() - 7
  AND {column} IS NULL
GROUP BY table_name, column_name
HAVING null_count > 0;
```

**Action**: Trace back to raw layer, fix join logic
...
#### Check 4: Regime Weight Merge Integrity

```sql
-- Every row must have a valid regime weight
SELECT 
  t.as_of_date,
  t.regime,
  t.row_weight,
  w.weight as expected_weight
FROM `training.zl_training_prod_allhistory_1m` t
LEFT JOIN `training.regime_weights` w ON t.regime = w.regime
WHERE t.as_of_date >= CURRENT_DATE() - 90
  AND (t.row_weight IS NULL OR t.row_weight != w.weight)
LIMIT 100;
```

**Required**: 100% regime weight alignment
