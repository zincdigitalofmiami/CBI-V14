-- Backup of signals.vw_technical_aggregates_comprehensive_daily
...
WITH price_technical AS (
  SELECT
    ...
    50.0 as zl_rsi_14,
    (close - LAG(close, 20) OVER (ORDER BY time)) / NULLIF(LAG(close, 20) OVER (ORDER BY time), 0) as zl_momentum_20d,
