...
Combines spatial pattern recognition (CNN) with temporal learning (LSTM).
"""
import numpy as np
...
def build_cnn_lstm_model(
    ...
    x = layers.Conv1D(filters=cnn_filters, kernel_size=3, activation='relu', padding='same')(inputs)
    x = layers.Dropout(dropout_rate)(x)
    x = layers.Conv1D(filters=cnn_filters, kernel_size=3, activation='relu', padding='same')(x)
    x = layers.Dropout(dropout_rate)(x)
    ...
    x = layers.LSTM(units=lstm_units, return_sequences=False, dropout=dropout_rate)(x)
...
def train_cnn_lstm_model(
    ...
    print(f"\n--- Training CNN-LSTM for {horizon} horizon ---")
    ...
    try:
        ...
        output_path = model_dir / f"cnn_lstm_{horizon}"
        model.save(output_path)
        
        import joblib
        joblib.dump(scaler, model_dir / f"cnn_lstm_{horizon}_scaler.pkl")
        
        print(f"✅ CNN-LSTM model saved to {output_path}")
