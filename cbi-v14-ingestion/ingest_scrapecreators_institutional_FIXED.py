#!/usr/bin/env python3
"""
CBI-V14 ScrapeCreators Institutional Intelligence - FIXED VERSION
Collects lobbying, congressional, analyst, and China state media intelligence
API Key: B1TOgQvMVSV6TDglqB8lJ2cirqi2
FIX: Corrected endpoint to /v1/twitter/user-tweets with 'handle' parameter
"""
...
class InstitutionalIntelligenceCollector:
    ...
    def fetch_twitter_user_posts(self, username: str, count: int = 10) -> List[Dict]:
        ...
        FIXED: Using correct endpoint /v1/twitter/user-tweets with 'handle' parameter
        """
        # CORRECTED ENDPOINT
        url = f"{SCRAPECREATORS_BASE_URL}/v1/twitter/user-tweets"
        params = {
            "handle": username,  # FIXED: Changed from 'username' to 'handle'
            "count": count
        }
        
        try:
            logger.info(f"  Fetching @{username} (timeout=45s)...")
            response = requests.get(
                url,
                headers=self.headers,
                params=params,
                timeout=45  # Increased timeout due to observed delays
            )
            
            if response.status_code == 200:
                data = response.json()
                tweets = data.get('tweets', [])  # FIXED: Changed from 'posts' to 'tweets'
                credits = data.get('credits_remaining', 'N/A')
                logger.info(f"    ✅ Retrieved {len(tweets)} tweets (Credits: {credits})")
                return tweets
            elif response.status_code == 404:
                logger.warning(f"    ⚠️ User @{username} not found")
                return []
            elif response.status_code == 400:
                logger.warning(f"    ⚠️ Bad request for @{username}: {response.json().get('message', 'Unknown error')}")
                return []
            elif response.status_code == 429:
                logger.warning(f"    ⚠️ Rate limited for @{username}")
                return []
            else:
                logger.warning(f"    ⚠️ API returned {response.status_code} for @{username}")
                logger.warning(f"    Response: {response.text[:200]}")
                return []
                
        except requests.exceptions.Timeout:
            logger.error(f"    ❌ Timeout fetching @{username} (45s exceeded)")
            return []
    ...
    def collect_category_intelligence(self, category: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"\nCollecting {category} intelligence...")
        
        for username in self.twitter_handles.get(category, []):
            tweets = self.fetch_twitter_user_posts(username, count=10)
            
            for tweet in tweets:
                relevance = self.analyze_tweet_relevance(tweet.get('text', ''), category)
                ...
                if relevance['total_relevance'] > 0:
                    intelligence.append({
                        ...
                        'influence_score': tweet.get('retweet_count', 0) * 2 + tweet.get('favorite_count', 0),
            ...
            # Rate limiting - be respectful (3 seconds between handles)
            time.sleep(3)
    ...
    def load_to_bigquery(self, category: str, records: List[Dict[str, Any]]) -> bool:
        ...
        if not records:
            logger.warning(f"  ⚠️ No records to load for {category}")
            return False
    ...
    def execute_complete_collection(self) -> bool:
        ...
        logger.info("=" * 70)
        logger.info("🚀 INSTITUTIONAL INTELLIGENCE COLLECTION (FIXED VERSION)")
        logger.info(f"API Key: {SCRAPECREATORS_API_KEY[:10]}...")
        logger.info(f"Endpoint: {SCRAPECREATORS_BASE_URL}/v1/twitter/user-tweets")
        logger.info("=" * 70)
