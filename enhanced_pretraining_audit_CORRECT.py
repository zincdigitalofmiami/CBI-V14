...
def run_enhanced_audit():
        ...
        try:
            ...
            if not result.empty:
                ...
                if r2 is not None:
                    ...
                    # Flag data leakage (impossible R² values)
                    if r2 > 0.999:
                        print(f"    ❌ CRITICAL: R² > 0.999 indicates DATA LEAKAGE!")
                        print(f"    This model is seeing the future - DO NOT USE!")
                        issues_found += 1
                    elif estimated_mape < 0.5:
                        print(f"    ⚠️ WARNING: MAPE < 0.5% is suspiciously good for commodities")
                        warnings += 1
                else:
                    print()
    ...
    if issues_found == 0:
        if warnings == 0:
            ...
            print(f"   - Production models: 1M, 3M, 6M are institutional-grade")
            print(f"   - Ignore 1W model (data leakage detected)")
            return True
        else:
