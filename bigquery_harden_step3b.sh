...
# Create partitioned table if missing (adds optional ticker STRING for clustering)
if ! exists_table soybean_prices_p; then
  bq mk --project_id="${PROJECT}" \
        --table \
        --time_partitioning_field time \
        --time_partitioning_type DAY \
        --clustering_fields ticker,close \
...
# Migrate rows
echo "-> Migrating data from soybean_prices to soybean_prices_p (idempotent)"
...
echo "-> Migrating data from weather_data to weather_data_p (idempotent)"
bq query --project_id="${PROJECT}" --use_legacy_sql=false --quiet "
...
echo "-> Migrating data from fed_rates to fed_rates_p (idempotent)"
bq query --project_id="${PROJECT}" --use_legacy_sql=false --quiet "
...
echo "-> Migrating data from economic_indicators to economic_indicators_p (idempotent)"
bq query --project_id="${PROJECT}" --use_legacy_sql=false --quiet "
