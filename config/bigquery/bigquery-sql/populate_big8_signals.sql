...
WITH weather_averages AS (
  SELECT
    DATE(date) as date,
    -- Brazil weather score (using avg temp between min/max)
    AVG(CASE
      WHEN region LIKE '%Brazil%' AND (temp_min + temp_max)/2 BETWEEN 20 AND 30
           AND precip_mm BETWEEN 50 AND 150 THEN 0.8  -- Good conditions
      WHEN region LIKE '%Brazil%' AND ((temp_min + temp_max)/2 < 15 OR (temp_min + temp_max)/2 > 35
           OR precip_mm < 25 OR precip_mm > 200) THEN 0.4  -- Poor conditions
      ELSE 0.6  -- Average conditions
    END) as brazil_score,
    -- Argentina weather score
    AVG(CASE
      WHEN region LIKE '%Argentina%' AND (temp_min + temp_max)/2 BETWEEN 15 AND 25
           AND precip_mm BETWEEN 40 AND 120 THEN 0.8  -- Good conditions
      WHEN region LIKE '%Argentina%' AND ((temp_min + temp_max)/2 < 10 OR (temp_min + temp_max)/2 > 30
           OR precip_mm < 20 OR precip_mm > 180) THEN 0.4  -- Poor conditions
      ELSE 0.6  -- Average conditions
...
-- China Relations Signal (simplified from social intelligence)
...
-- Tariff Threat Signal (simplified from social intelligence)
...
FULL OUTER JOIN (
  ...
  FROM `cbi-v14.staging.comprehensive_social_intelligence`
  WHERE SAFE.PARSE_TIMESTAMP('%a %b %d %H:%M:%S +0000 %Y', created_at) IS NOT NULL
    AND (LOWER(content) LIKE '%geopolitical%' OR LOWER(content) LIKE '%crisis%')
  GROUP BY DATE(SAFE.PARSE_TIMESTAMP('%a %b %d %H:%M:%S +0000 %Y', created_at))
) n ON v.date = n.date
...
CREATE OR REPLACE TABLE `cbi-v14.models_v4.temp_hidden_correlation_historical` AS
SELECT
  DATE(s.time) as date,
  COALESCE(
