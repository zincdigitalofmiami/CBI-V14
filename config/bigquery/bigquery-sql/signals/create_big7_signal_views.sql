...
-- =====================================================
-- BIG 7 SIGNAL VIEWS FOR CBI-V14
...
WITH brazil_conditions AS (
    SELECT 
        DATE(date) as date,
        AVG(CASE 
            WHEN precipitation_mm < 50 THEN 0.6   -- Drought
            WHEN precipitation_mm > 200 THEN 0.9  -- Good
            ELSE 0.75  -- Normal
        END) as brazil_score
    FROM `cbi-v14.forecasting_data_warehouse.weather_data`
    WHERE station_id LIKE 'BR%'
...
argentina_conditions AS (
    SELECT 
        DATE(date) as date,
        AVG(CASE 
            WHEN precipitation_mm < 40 THEN 0.65   -- Drought
            WHEN precipitation_mm > 150 THEN 0.85  -- Good  
            ELSE 0.75  -- Normal
        END) as argentina_score
    FROM `cbi-v14.forecasting_data_warehouse.weather_data`
    WHERE station_id LIKE 'AR%'
      AND date >= DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY)
    GROUP BY DATE(date)
)
SELECT 
...
CREATE OR REPLACE VIEW `cbi-v14.signals.vw_china_relations_big7` AS
...
CREATE OR REPLACE VIEW `cbi-v14.signals.vw_tariff_threat_big7` AS
WITH tariff_data AS (
    SELECT 
        DATE(created_at) as date,
        -- 7-day rolling tariff mentions
        COUNT(CASE WHEN LOWER(content) LIKE '%tariff%' THEN 1 END) OVER (
            ORDER BY DATE(created_at) 
            ROWS BETWEEN 6 PRECEDING AND CURRENT ROW
        ) as tariff_mentions_7d,
        
        -- Trade tension component
        AVG(CASE 
...
-- Formula: vix/20 * 0.4 + trump_correlation * 0.3 + panama_delays/15 * 0.2 + em_stress * 0.1
CREATE OR REPLACE VIEW `cbi-v14.signals.vw_geopolitical_volatility_big7` AS
...
CREATE OR REPLACE VIEW `cbi-v14.signals.vw_biofuel_cascade_big7` AS
...
CREATE OR REPLACE VIEW `cbi-v14.signals.vw_hidden_correlation_big7` AS
...
-- MASTER BIG 7 COMPOSITE VIEW
CREATE OR REPLACE VIEW `cbi-v14.api.vw_big7_composite_signal` AS
WITH latest_signals AS (
    SELECT 
        CURRENT_DATE() as date,
        
        -- VIX Stress
        (SELECT vix_stress_score FROM `cbi-v14.signals.vw_vix_stress_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_vix_stress_big7`)) as vix_stress,
        
        -- Harvest Pace
        (SELECT harvest_pace_score FROM `cbi-v14.signals.vw_harvest_pace_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_harvest_pace_big7`)) as harvest_pace,
        
        -- China Relations
        (SELECT china_relations_score FROM `cbi-v14.signals.vw_china_relations_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_china_relations_big7`)) as china_relations,
        
        -- Tariff Threat
        (SELECT tariff_threat_score FROM `cbi-v14.signals.vw_tariff_threat_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_tariff_threat_big7`)) as tariff_threat,
        
        -- GVI
        (SELECT gvi_score FROM `cbi-v14.signals.vw_geopolitical_volatility_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_geopolitical_volatility_big7`)) as geopolitical_volatility,
        
        -- BSC
        (SELECT bsc_score FROM `cbi-v14.signals.vw_biofuel_cascade_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_biofuel_cascade_big7`)) as biofuel_cascade,
        
        -- HCI
        (SELECT hci_score FROM `cbi-v14.signals.vw_hidden_correlation_big7` 
         WHERE date = (SELECT MAX(date) FROM `cbi-v14.signals.vw_hidden_correlation_big7`)) as hidden_correlation
),
crisis_calc AS (
    SELECT 
        *,
