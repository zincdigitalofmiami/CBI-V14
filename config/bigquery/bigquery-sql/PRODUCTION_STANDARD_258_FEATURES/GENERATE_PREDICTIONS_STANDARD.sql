...
-- ============================================
-- GENERATE PRODUCTION FORECASTS - STANDARDIZED 258 FEATURES
-- ============================================
...
forecast_1w AS (
  ...
  FROM ML.PREDICT(
    MODEL `cbi-v14.models_v4.bqml_1w_all_features`,  -- ✅ STANDARDIZED MODEL
...
forecast_1m AS (
  ...
  FROM ML.PREDICT(
    MODEL `cbi-v14.models_v4.bqml_1m_all_features`,  -- ✅ STANDARDIZED MODEL
    (SELECT * FROM latest_data)
  )
),
...
forecast_3m AS (
  ...
  FROM ML.PREDICT(
    MODEL `cbi-v14.models_v4.bqml_3m_all_features`,  -- ✅ STANDARDIZED MODEL
    (SELECT * FROM latest_data)
  )
),
...
forecast_6m AS (
  ...
  FROM ML.PREDICT(
    MODEL `cbi-v14.models_v4.bqml_6m_all_features`,  -- ✅ STANDARDIZED MODEL
    (SELECT * FROM latest_data)
  )
),
...
historical_mape AS (
  SELECT '1W' as horizon, 1.21 as mape_historical, 'bqml_1w_all_features' as model_name
  UNION ALL
  SELECT '1M', 1.29, 'bqml_1m_all_features'
  UNION ALL
  SELECT '3M', 0.70, 'bqml_3m_all_features'
  UNION ALL
  SELECT '6M', 1.21, 'bqml_6m_all_features'
),
