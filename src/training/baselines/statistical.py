...
def load_training_data(horizon):
    """Load cached training data for horizon"""
...
def train_arima(train, val, target_col, horizon):
    """Train ARIMA model"""
    print(f"\n{'='*60}")
    ...
    with mlflow.start_run(run_name=f"arima_{horizon}"):
        ...
        model_path = f"{MODELS_DIR}/arima_{horizon}.pkl"
        import pickle
...
def train_auto_arima(train, val, target_col, horizon):
    ...
    with mlflow.start_run(run_name=f"auto_arima_{horizon}"):
        ...
        # Save model
        model_path = f"{MODELS_DIR}/auto_arima_{horizon}.pkl"
        import pickle
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
...
def train_prophet(train, val, target_col, horizon):
    ...
    with mlflow.start_run(run_name=f"prophet_{horizon}"):
        ...
        # Save model
        model_path = f"{MODELS_DIR}/prophet_{horizon}.pkl"
        import pickle
        with open(model_path, 'wb') as f:
            pickle.dump(model, f)
...
def train_exponential_smoothing(train, val, target_col, horizon):
    ...
    with mlflow.start_run(run_name=f"exp_smoothing_{horizon}"):
        ...
        # Save model
        model_path = f"{MODELS_DIR}/exp_smoothing_{horizon}.pkl"
        import pickle
        with open(model_path, 'wb') as f:
            pickle.dump(model_fit, f)
...
def main():
    ...
    try:
        _, results['arima'] = train_arima(train, val, target_col, horizon)
    except Exception as e:
        print(f"❌ ARIMA failed: {e}")
        results['arima'] = None
    
    try:
        _, results['auto_arima'] = train_auto_arima(train, val, target_col, horizon)
    except Exception as e:
        print(f"❌ Auto-ARIMA failed: {e}")
        results['auto_arima'] = None
    
    try:
        _, results['prophet'] = train_prophet(train, val, target_col, horizon)
    except Exception as e:
        print(f"❌ Prophet failed: {e}")
        results['prophet'] = None
    
    try:
        _, results['exp_smoothing'] = train_exponential_smoothing(train, val, target_col, horizon)
    except Exception as e:
