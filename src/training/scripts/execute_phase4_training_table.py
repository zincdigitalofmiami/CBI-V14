#!/usr/bin/env python3
"""
PHASE 4: Create Full Training Table with All 159 Features
"""
...
print("="*80)
print("PHASE 4: CREATING FULL TRAINING TABLE (159 FEATURES)")
print("="*80)
print(f"Start Time: {datetime.now().isoformat()}\n")

# Read the original view definition to get all features
print("Building comprehensive training table with all feature JOINs...")
...
SELECT 
    ...
    -- PRICE FEATURES (14) - FROM MATERIALIZED TABLE (NO WINDOW FUNCTIONS)
    p.zl_price_current,
    p.zl_price_lag1,
    p.zl_price_lag7,
    p.zl_price_lag30,
    ...
    -- SEASONALITY (3) - REMOVED DUE TO CORRELATED SUBQUERY IN VIEW
    -- Will add back when view is fixed
...
-- LEFT JOIN `cbi-v14.models.vw_seasonality_features` sz ON p.date = sz.date  -- Skipped: has correlated subqueries
...
try:
    ...
    print(f"✅ Rows: {val_result.total_rows}")
    ...
    """
    schema_result = list(client.query(schema_query).result())[0]
    print(f"✅ Columns: {schema_result.column_count}")
    
    if schema_result.column_count == 156:
        print("\n🎯 SUCCESS: All 156 columns present! (3 seasonality columns skipped)")
    else:
        print(f"\n⚠️ WARNING: Expected 156 columns, got {schema_result.column_count}")
