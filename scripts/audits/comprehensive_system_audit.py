...
# 1. DISCOVER ALL TABLES IN FORECASTING_DATA_WAREHOUSE
print("\n" + "="*80)
print("1. FORECASTING_DATA_WAREHOUSE - ALL TABLES")
print("="*80)
...
try:
    ...
    info.append(f"Found {len(fw_tables)} tables in forecasting_data_warehouse")
    
except Exception as e:
    print(f"❌ Error listing tables: {str(e)[:100]}")
    issues.append(f"Could not list forecasting_data_warehouse tables: {str(e)[:100]}")

# 2. DISCOVER ALL TABLES IN MODELS_V4
print("\n" + "="*80)
print("2. MODELS_V4 - ALL TABLES")
print("="*80)
...
try:
    ...
    models_tables = []
    for row in result:
        models_tables.append(row.table_name)
        print(f"  📊 {row.table_name:50} | {row.column_count:3} cols")
    
    info.append(f"Found {len(models_tables)} tables in models_v4")
    
except Exception as e:
    print(f"❌ Error listing tables: {str(e)[:100]}")
    issues.append(f"Could not list models_v4 tables: {str(e)[:100]}")
...
key_tables = {
    'forecasting_data_warehouse': [
        ...
        'soybean_meal_prices', 'palm_oil_prices', 'canola_oil_prices',
        'crude_oil_prices', 'natural_gas_prices', 'gold_prices', 'silver_prices',
        'copper_prices', 'vix_daily', 'sp500_prices', 'usd_index_prices',
        'treasury_prices', 'cftc_cot', 'economic_indicators',
        'china_soybean_imports', 'news_intelligence', 'news_advanced',
        'social_sentiment', 'trump_policy_intelligence', 'weather_data',
        'biofuel_prices', 'baltic_dry_index'
    ],
    'models_v4': [
        'production_training_data_1w', 'production_training_data_1m',
        'production_training_data_3m', 'production_training_data_6m',
        'production_training_data_12m', 'trump_rich_2023_2025',
        'pre_crisis_2000_2007_historical', 'crisis_2008_historical',
        'recovery_2010_2016_historical', 'trade_war_2017_2019_historical'
    ]
}
...
try:
    ...
    for row in result:
        ...
        try:
            ...
            test_query = f"SELECT COUNT(*) as cnt FROM `cbi-v14.forecasting_data_warehouse.{row.table_name}` LIMIT 1"
    ...
    info.append(f"Found {len(views)} views in forecasting_data_warehouse")
...
try:
    ...
    info.append(f"Found {len(signal_tables)} tables and {len(signal_views)} views in signals")
    
except Exception as e:
    if "Not found: Dataset" in str(e):
        print("  ⚠️ Signals dataset does not exist")
        warnings.append("Signals dataset not found")
    else:
        print(f"❌ Error listing signals: {str(e)[:100]}")
        warnings.append(f"Could not list signals dataset: {str(e)[:100]}")
...
critical_checks = [
    ('cftc_cot', 'forecasting_data_warehouse', 'report_date', 2006, 2025, 5000),
    ('china_soybean_imports', 'forecasting_data_warehouse', 'date', 2017, 2025, 500),
    ('baltic_dry_index', 'forecasting_data_warehouse', 'date', 2000, 2025, 5000),
    ('argentina_soybean_exports', 'forecasting_data_warehouse', 'date', 2010, 2025, 1000),
    ('brazil_soybean_exports', 'forecasting_data_warehouse', 'date', 2010, 2025, 1000),
]
